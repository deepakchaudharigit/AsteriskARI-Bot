#!/usr/bin/env python3
"""
Script to create audio files for NPCL IVR system using TTS
"""
...
def create_audio_file(text, filename, language='en'):
...
def main():
    ...
    for filename, text in audio_files.items():
        create_audio_file(text, filename)
    
    print(f"\nCreated {len(audio_files)} audio files in {sounds_dir}")
    print("\nAudio files are ready for Asterisk IVR system!")
