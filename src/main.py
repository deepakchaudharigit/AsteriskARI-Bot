...
Provides multiple interaction modes including chat, voice, and combined modes with OpenAI integration.
...
def print_banner():
    ...
    print("🎆 Powered by OpenAI GPT-4 Realtime")
...
def speak_text_enhanced(text, language_code="en-IN", voice=None):
    """Enhanced TTS function with high-quality OpenAI TTS"""
    try:
        # Use the simple enhanced TTS module with OpenAI TTS-1-HD
        from voice_assistant.audio.simple_enhanced_tts import speak_text_enhanced as enhanced_speak
        return enhanced_speak(text, voice)
    except ImportError:
...
def start_multilingual_chat_mode(api_key, language_config):
    ...
    # Force Enhanced TTS with Whisper voice
    tts_available = False
    enhanced_tts_available = False
    
    try:
        # Try simple enhanced TTS first
        from voice_assistant.audio.simple_enhanced_tts import SimpleEnhancedTTS
        
        # Initialize with fable voice (warm, customer-friendly)
        tts_engine = SimpleEnhancedTTS()
        
        print(f"✅ Enhanced TTS initialized successfully")
        print(f"🎵 Voice: {tts_engine.voice_model} - Warm, friendly voice - good for customer service")
        print(f"🎛️  Model: {tts_engine.tts_model}")
        ...
        if test_success:
            ...
            tts_available = True
            print("🔊 Voice output enabled (Enhanced OpenAI TTS-HD with Whisper)")
            print(f"🎵 Voice: {tts_engine.voice_model} - Warm, friendly voice - good for customer service")
            print(f"🎛️  Model: {tts_engine.tts_model}")
        else:
            raise Exception("Enhanced TTS test failed")
        
    except Exception as e:
        print(f"❌ Enhanced TTS failed: {e}")
        # Try fallback to basic TTS instead of failing completely
        try:
            import pyttsx3
            ...
            tts_available = True
            enhanced_tts_available = False
            print("🔊 Voice output enabled (Basic TTS Mode)")
            print("💡 Enhanced TTS not available, using basic voice")
        except Exception as e2:
            print(f"❌ All TTS failed: {e2}")
            print("❌ OFFLINE MODE DISABLED - TTS required")
            print("💡 Please install: pip install openai pygame")
            return
    ...
    try:
        ...
        try:
            ...
            if "quota" in str(e).lower() or "429" in str(e):
                print(f"❌ API quota exceeded. Offline mode disabled.")
                print(f"💡 Please check your OpenAI API quota and try again.")
                return
            else:
                print(f"❌ API error: {e}")
                return
        ...
        try:
            ...
            ai_welcome = initial_response.choices[0].message.content
            
            print(f"🤖 NPCL Assistant: {ai_welcome}")
            print()
            
            # Speak the welcome message using enhanced TTS
            if tts_available:
                if enhanced_tts_available:
                    speak_text_enhanced(ai_welcome, lang_code)
                else:
                    speak_text_robust(ai_welcome, lang_code)
            
        except Exception as e:
            if "quota" in str(e).lower() or "429" in str(e):
                print(f"❌ API quota exceeded during welcome. Offline mode disabled.")
                return
            else:
                ...
                if tts_available:
                    if enhanced_tts_available:
                        speak_text_enhanced(fallback_welcome, lang_code)
                    else:
        ...
        while True:
            try:
                ...
                if user_input.lower() in ['quit', 'exit', 'bye', 'goodbye', 'बाहर निकलें', 'প্রস্থান', 'έξοδος']:
                    ...
                    if tts_available:
                        if enhanced_tts_available:
                            speak_text_enhanced(goodbye_msg, lang_code)
                        else:
                ...
                try:
                    ...
                    # Always speak the response using enhanced TTS
                    if tts_available:
                        if enhanced_tts_available:
                            speak_text_enhanced(response_text, lang_code)
                        else:
                    ...
                    if "quota" in str(e).lower() or "429" in str(e):
                        ...
                        if tts_available:
                            if enhanced_tts_available:
                                speak_text_enhanced(fallback_response, lang_code)
                            else:
        ...
        print("3. openai package installed")
...
def start_offline_mode(language_config):
    ...
    # Test Enhanced TTS availability
    tts_available = False
    enhanced_tts_available = False
    
    try:
        # Try enhanced TTS first
        from voice_assistant.audio.enhanced_tts import speak_text_enhanced as enhanced_speak
        enhanced_tts_available = True
        tts_available = True
        ...
        print(f"⚠️  Enhanced TTS setup failed: {e}")
        try:
            import pyttsx3
            test_engine = pyttsx3.init()
            test_engine.stop()
    ...
    if tts_available:
        if enhanced_tts_available:
            speak_text_enhanced(welcome, lang_code)
        else:
...
def start_ari_bot_mode():
    """Start ARI bot mode directly"""
    print("=" * 70)
    ...
    # Force enhanced TTS setup
    print("🎤 Setting up Enhanced Voice System:")
    
    try:
        # Try simple enhanced TTS first
        from voice_assistant.audio.simple_enhanced_tts import SimpleEnhancedTTS
        
        # Initialize simple enhanced TTS with fable voice (customer-friendly)
        tts = SimpleEnhancedTTS()
        
        print(f"✅ Enhanced TTS initialized successfully")
        print(f"🎵 Voice: {tts.voice_model} - Warm, friendly voice - good for customer service")
        ...
        try:
            import pyttsx3
            engine = pyttsx3.init()
            engine.say("NPCL ARI Bot starting with basic voice.")
            engine.runAndWait()
            engine.stop()
            del engine
            ...
            print(f"❌ All TTS failed: {e2}")
            print("⚠️  Continuing without voice...")
    
    # Start ARI server
    try:
        print("\n🚀 Starting Asterisk ARI Server...")
        
        # Import and run the ARI server
        import subprocess
        import sys
        
        # Run the realtime server
        result = subprocess.run([
            sys.executable, "src/run_realtime_server.py"
        ], cwd=".")
...
def main():
    """Main application entry point - ARI Bot Mode"""
    # Check if running as ARI bot
    if len(sys.argv) > 1 and sys.argv[1] == '--ari-bot':
        return start_ari_bot_mode()
    ...
    if not api_key_valid:
        ...
        print("2. Get your OpenAI API key from: https://platform.openai.com/")
