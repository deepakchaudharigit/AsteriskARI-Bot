"""
Modern Voice Assistant with OpenAI Realtime API
...
class ModernVoiceAssistant:
    """Modern voice assistant with OpenAI Realtime API and real-time capabilities"""
    ...
    def __init__(self, 
        ...
        # Fallback components for when Realtime API is not available
        ...
        try:
            ...
            if self._connect_to_live_api():
                ...
                if (self.live_audio_streamer.start_streaming(self.loop) and 
                    ...
                    logger.info("Connected to OpenAI Realtime API - Real-time mode enabled")
                else:
            ...
            if self.is_live_mode:
                ...
                logger.info("Sending welcome message to Realtime API for native voice")
    ...
    def _connect_to_live_api(self) -> bool:
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self.openai_realtime.connect(), self.loop
            )
            connected = future.result(timeout=15.0)  # Increased timeout
            
            if connected:
                logger.info("Realtime API WebSocket connected")
                
                # Start conversation session
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_realtime.start_conversation(), self.loop
                )
                session_id = future.result(timeout=5.0)
                logger.info(f"Started Realtime API session: {session_id}")
                
                return True
            else:
                logger.warning("Realtime API connection failed")
                return False
            
        except asyncio.TimeoutError:
            logger.error("Realtime API connection timeout - API may not be available")
            return False
        except Exception as e:
    ...
    def _disconnect_from_live_api(self):
        ...
        try:
            if self.loop and self.loop.is_running():
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_realtime.disconnect(), self.loop
                )
    ...
    def _setup_live_api_handlers(self):
        """Setup event handlers for OpenAI Realtime API"""
        
        # Setup complete handler
        async def handle_setup_complete(event_data):
            logger.info("Realtime API setup completed successfully")
            self._set_state(ModernAssistantState.CONNECTED)
        ...
        # Register handlers
        self.openai_realtime.register_event_handler("setup_complete", handle_setup_complete)
        self.openai_realtime.register_event_handler("audio_response", handle_audio_response)
        self.openai_realtime.register_event_handler("audio_response_done", handle_audio_response_done)
        self.openai_realtime.register_event_handler("interrupted", handle_interrupted)
        self.openai_realtime.register_event_handler("error", handle_error)
        
        logger.debug("Realtime API event handlers setup complete")
    
    def _send_text_to_live_api(self, text: str):
        """Send text to Realtime API for native voice synthesis"""
        try:
            if not self.is_live_mode or not self.loop:
                raise RuntimeError("Realtime API not available")
            
            # Send text message to Realtime API for voice synthesis
            future = asyncio.run_coroutine_threadsafe(
                self._send_text_message_to_live_api(text), self.loop
            )
            future.result(timeout=2.0)
            logger.debug(f"Text sent to Realtime API: {text[:50]}...")
            
        except Exception as e:
            logger.error(f"Failed to send text to Realtime API: {e}")
            raise
    
    async def _send_text_message_to_live_api(self, text: str):
        """Send text message to Realtime API (async)"""
        try:
            ...
            await self.openai_realtime._send_event(message)
            logger.debug("Text message sent to Realtime API")
            
        except Exception as e:
            logger.error(f"Error sending text message to Realtime API: {e}")
            raise
    
    def _send_welcome_to_live_api(self, welcome_message: str):
        """Send welcome message to Realtime API to get native voice response"""
        try:
            if not self.is_live_mode or not self.loop:
                logger.warning("Realtime API not available, using traditional TTS")
                self._speak_response(welcome_message)
                return
            ...
            logger.info("Welcome message sent to Realtime API")
            
        except Exception as e:
            logger.error(f"Failed to send welcome to Realtime API: {e}")
    ...
    async def _send_system_message_to_live_api(self, message: str):
        """Send system message to Realtime API (async)"""
        try:
            ...
            await self.openai_realtime._send_event(content)
            logger.debug("System message sent to Realtime API")
            
        except Exception as e:
            logger.error(f"Error sending system message to Realtime API: {e}")
            raise
    
    def _process_live_conversation_turn(self) -> bool:
        """Process conversation turn using Realtime API"""
        try:
            # In Realtime API mode, audio is continuously streamed
            # We just need to wait for user interaction or check for exit commands
            self._set_state(ModernAssistantState.LISTENING)
            
            # Wait for a short period to allow Realtime API to process
            time.sleep(0.1)
    ...
    def _process_traditional_conversation_turn_with_text(self, user_text: str) -> bool:
        ...
        try:
            ...
            # Use basic OpenAI client for fallback
            from ..ai.openai_realtime_client import OpenAIRealtimeClient
            openai_client = OpenAIRealtimeClient(api_key=self.settings.openai_api_key)
            response = openai_client.generate_response(user_text)
    ...
    def _speak_response(self, response: str):
        ...
        if self.is_live_mode:
            # In Realtime API mode, send text to Realtime API for native voice synthesis
            try:
                self._send_text_to_live_api(response)
            except Exception as e:
                logger.error(f"Realtime API TTS failed, falling back to traditional: {e}")
                # Fallback to traditional TTS
                if not self.tts.speak(response):
                    logger.warning("Both Realtime API and traditional TTS failed")
        else:
            # Traditional mode - use local TTS
            if not self.tts.speak(response):
    ...
    def _get_welcome_message(self) -> str:
        """Get welcome message"""
        if self.is_live_mode:
            return f"Hello! I'm {self.settings.assistant_name}, your modern voice assistant powered by OpenAI Realtime API with real-time conversation and interruption support. I'm ready to have a natural conversation with you. What would you like to talk about?"
        else:
    ...
    def reset_conversation(self):
        ...
        if self.is_live_mode:
            # Reset Realtime API session
            try:
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_realtime.clear_audio_buffer(), self.loop
                )
                future.result(timeout=2.0)
            except Exception as e:
                logger.error(f"Error resetting Realtime API session: {e}")
        else:
    ...
    def interrupt_response(self):
        ...
        if self.is_live_mode and self.is_assistant_speaking:
            try:
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_realtime.cancel_response(), self.loop
                )
    ...
    def get_live_api_status(self) -> Dict[str, Any]:
        """Get Realtime API connection status"""
        if self.is_live_mode:
            return self.openai_realtime.get_connection_status()
        else:
