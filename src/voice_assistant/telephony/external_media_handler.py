...
Handles real-time audio communication between Asterisk and OpenAI Realtime API
...
from ..ai.ai_client_factory import AIClientFactory, create_ai_client
# Using OpenAI client factory for AI integration
from ..ai.openai_realtime_client_enhanced import OpenAIRealtimeClientEnhanced
...
class ExternalMediaHandler:
    """Handles multiple external media connections and integrates with AI providers"""
    ...
    def __init__(self, session_manager: SessionManager, ai_client = None):
        ...
        # Register AI client event handlers for audio responses
        if hasattr(self.ai_client, 'register_event_handler'):
    ...
    async def _handle_audio_from_asterisk(self, event_data: Dict[str, Any]):
        ...
        try:
            ...
            # Send audio to AI client (OpenAI)
            if hasattr(self.ai_client, 'is_connected') and self.ai_client.is_connected:
                await self.ai_client.send_audio_chunk(audio_data)
            elif hasattr(self.ai_client, 'send_audio_chunk'):
                # For enhanced OpenAI client
                await self.ai_client.send_audio_chunk(audio_data)
            
            # Also check for queued audio output from AI client
            if hasattr(self.ai_client, 'get_audio_output'):
                while True:
                    ai_audio = self.ai_client.get_audio_output()
                    if ai_audio is None:
                        break
                    # Send AI audio back to caller
                    connection = self.connections.get(channel_id)
                    if connection and connection.is_connected:
                        await connection.send_audio(ai_audio)
                        logger.debug(f"Sent queued AI audio to channel {channel_id}")
    ...
    async def _handle_ai_audio_response(self, event_data: Dict[str, Any]):
        """Handle audio response from AI client"""
        try:
            ...
            for channel_id, connection in self.connections.items():
                if connection.is_connected:
                    ...
                    logger.debug(f"Sent AI audio response to channel {channel_id}")
            
        except Exception as e:
            logger.error(f"Error handling AI audio response: {e}")
    
    async def _handle_ai_response_done(self, event_data: Dict[str, Any]):
        """Handle AI response completion"""
        try:
            logger.debug("AI response completed")
            # Could add any cleanup or state management here
            
        except Exception as e:
            logger.error(f"Error handling AI response done: {e}")
