...
# Optional import for audio processing (not needed for OpenAI Real-time API)
...
def _import_audio_libs():
    ...
    global PYDUB_AVAILABLE, AudioSegment
...
class AudioUtils:
    ...
    def convert_audio_format(input_path: str, output_path: str, 
        ...
        """
        if not _import_audio_libs():
            logger.warning("pydub not available - audio conversion disabled (using OpenAI Real-time API)")
            return False
