"""
Real-time microphone streaming for OpenAI Realtime API
...
class LiveAudioStreamer:
    """Streams microphone audio to OpenAI Realtime API"""
    
    def __init__(self, openai_client, config: MicrophoneConfig = None):
        ...
        try:
            ...
            if self.openai_client.is_connected:
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_client.send_audio_chunk(audio_data),
                    self.loop
                )
    ...
    def _on_speech_end(self):
        ...
        # Commit audio buffer
        if self.is_streaming and self.loop and self.openai_client.is_connected:
            try:
                future = asyncio.run_coroutine_threadsafe(
                    self.openai_client.commit_audio_buffer(),
                    self.loop
                )
