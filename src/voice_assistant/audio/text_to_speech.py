"""
Text-to-speech module using OpenAI Text-to-Speech (gTTS)
"""
...
# Optional imports for local TTS (not needed for OpenAI Real-time API)
# Only import if explicitly needed to avoid ffmpeg warnings
...
def _import_audio_libs():
    """Lazy import audio libraries only when needed"""
    global PYDUB_AVAILABLE, AudioSegment, play, gTTS
...
class TextToSpeech:
    ...
    def __init__(self):
        ...
        # For OpenAI Real-time API, we don't need local TTS
        # Only initialize if explicitly requested
        logger.info("TextToSpeech initialized (local TTS disabled for OpenAI Real-time API)")
    ...
    def speak(self, text: str, save_to_file: Optional[str] = None) -> bool:
        ...
        """
        # For OpenAI Real-time API, we don't use local TTS
        logger.info("Using OpenAI Real-time API for voice synthesis")
        return True  # Return success since OpenAI handles TTS
