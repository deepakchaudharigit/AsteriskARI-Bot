#!/usr/bin/env python3
"""
Real-Time Voice Chatbot with Fixed Post-Interruption Flow
Features:
- Instant interruption with keywords like "stop", "hold on", "ok"
- Proper conversation continuation after interruption
...
class RealTimeVoiceChatbotFixed:
    def __init__(self):
        """Initialize the real-time voice chatbot with fixed interruption flow."""
        ...
        self.conversation_paused = False  # NEW: Track if conversation is paused
    ...
    def _reset_conversation_state(self):
        """Reset conversation state after interruption."""
        self.should_interrupt = False
        self.processing_speech = False
        ...
        self.interrupt_detected_event.clear()
    ...
    def print_instructions(self):
        ...
        print(f"\n{Fore.CYAN}{'='*70}")
        print(f"{Fore.CYAN}🎙️  REAL-TIME VOICE CHATBOT (FIXED FLOW)")
        ...
        print(f"{Fore.WHITE}• ✅ Fixed conversation flow after interruption")
    ...
    def run(self):
        """Main conversation loop with real-time interruption and fixed flow."""
        try:
            ...
            while self.is_running:
                try:
                    ...
                    # Handle any interruptions that occurred during speech
                    ...
                    # If interruption occurred, reset state and continue conversation
                    if interruption_occurred:
                        self._reset_conversation_state()
                        self._print_status("✅ Ready to continue conversation...", "success")
