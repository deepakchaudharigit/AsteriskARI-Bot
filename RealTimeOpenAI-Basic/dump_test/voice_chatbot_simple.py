#!/usr/bin/env python3
"""
Real-time Voice Chatbot with OpenAI (Simplified Version)
Features:
- Real-time voice input/output
- Modern ChatGPT voice capabilities
- Terminal-based interface
- Simplified controls (no keyboard library dependency)
"""
...
class VoiceChatbot:
        ...
        try:
            ...
            with self.client.audio.speech.with_streaming_response.create(
                ...
                with tempfile.NamedTemporaryFile(suffix=".mp3", delete=False) as temp_audio:
                    ...
                    try:
                        # Try to play with pydub (requires ffmpeg)
                        from pydub import AudioSegment
                        from pydub.playback import play
                        audio_segment = AudioSegment.from_mp3(temp_audio.name)
                        play(audio_segment)
                    except Exception as pydub_error:
                        # Fallback: try with system command
                        try:
                            import subprocess
                            subprocess.run(["afplay", temp_audio.name], check=True)  # macOS
                        except (subprocess.CalledProcessError, FileNotFoundError):
                            try:
                                subprocess.run(["mpg123", temp_audio.name], check=True)  # Linux
                            except (subprocess.CalledProcessError, FileNotFoundError):
                                self._print_status("⚠️ Audio playback not available. Install ffmpeg for audio support.", "warning")
    ...
    def print_instructions(self):
        ...
        print(f"\n{Fore.CYAN}{'='*60}")
        print(f"{Fore.CYAN}🎙️  REAL-TIME VOICE CHATBOT (Simplified)")
        ...
        print(f"{Fore.WHITE}• Type {Fore.YELLOW}'quit'{Fore.WHITE} or {Fore.YELLOW}'exit'{Fore.WHITE} to end the conversation")
