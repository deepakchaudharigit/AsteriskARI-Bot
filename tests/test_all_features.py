...
class TestCompleteFeatureSet:
    ...
    async def test_complete_call_workflow(self, test_settings):
        ...
        openai_client = OpenAIRealtimeClient(api_key="test-key")
        external_media_handler = ExternalMediaHandler(session_manager, openai_client)
        ...
        try:
            ...
            await openai_client.send_audio_chunk(speech_audio)
            await openai_client.commit_audio_buffer()
            await openai_client.create_response()
            events.add_event("openai_response_requested")
            ...
            expected_events = [
                ...
                "turn_added", "openai_response_requested", 
    ...
    @pytest.mark.asyncio
    async def test_real_time_audio_pipeline(self, audio_config, openai_config, performance_thresholds):
        ...
        async def audio_pipeline(data):
            ...
            # Send to Gemini
            success = await openai_client.send_audio_chunk(audio_data)
            if success:
        ...
        assert metrics["peak_memory_mb"] < performance_thresholds["memory_usage_mb"]  # Use configurable threshold
    ...
    @pytest.mark.asyncio
    async def test_error_handling_and_recovery(self, audio_config, openai_config):
        ...
        try:
            ...
            openai_client.is_connected = False
            
            success = await openai_client.send_audio_chunk(b"test_audio")
    ...
    @pytest.mark.asyncio
    async def test_performance_under_load(self, audio_config, openai_config, performance_thresholds):
        ...
        try:
            ...
            async def process_session_audio(session_id, session_index):
                for chunk_index in range(50):  # 1 second per session
                    ...
                    # Send to Gemini
                    await openai_client.send_audio_chunk(audio_chunk)
    ...
    def test_configuration_validation(self, test_settings):
        ...
        assert openai_config.voice == "Puck"
        assert openai_config.input_audio_format == "pcm16"
        assert openai_config.output_audio_format == "pcm16"
...
class TestFeatureIntegration:
    ...
    @pytest.mark.asyncio
    async def test_interruption_handling_workflow(self, audio_config, openai_config):
