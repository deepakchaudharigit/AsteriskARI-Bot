...
class TestPerformanceMonitor:
    ...
    def test_end_operation_success(self):
        ...
        operation_id = self.monitor.start_operation("openai_api")
        time.sleep(0.01)  # Small delay to measure latency
        
        self.monitor.end_operation(operation_id, "openai_api", success=True)
        
        # Check operation is removed from pending
        assert operation_id not in self.monitor.pending_operations
        
        # Check metrics updated
        assert self.monitor.metrics.openai_api_calls == 1
        assert self.monitor.metrics.openai_api_latency > 0
        
        # Check latency history
        assert len(self.monitor.latency_history["openai_api"]) == 1
    ...
    def test_end_nonexistent_operation(self):
        ...
        assert self.monitor.metrics.openai_api_calls == 0
    ...
    def test_get_current_metrics(self):
        ...
        operation_id = self.monitor.start_operation("openai_api")
        time.sleep(0.01)
        self.monitor.end_operation(operation_id, "openai_api", True)
        ...
        required_fields = [
            ...
            "openai_api_calls", "active_sessions", "total_sessions",
        ...
        assert metrics["openai_api_calls"] == 1
    ...
    async def test_full_monitoring_cycle(self):
        ...
        try:
            ...
            for i in range(3):
                ...
                # Simulate API call
                op_id = monitor.start_operation("openai_api")
                await asyncio.sleep(0.01)
                monitor.end_operation(op_id, "openai_api", True)
            ...
            # Verify results
            assert metrics["openai_api_calls"] == 3
