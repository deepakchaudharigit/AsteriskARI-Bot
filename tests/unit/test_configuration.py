...
class TestVoiceAssistantSettings:
    ...
    @pytest.mark.skip(reason="File permission issue in test environment")
    def test_default_settings(self):
        ...
        try:
            # Move .env file temporarily to test pure defaults
            if env_existed:
                import shutil; shutil.copy2(env_file, temp_env_file) if env_file.exists() else None; env_file.unlink() if env_file.exists() else None
            
            # Clear all environment variables that could affect settings
            env_vars_to_clear = [
                "GEMINI_MODEL", "GEMINI_LIVE_MODEL", "GEMINI_VOICE",
                "ASSISTANT_NAME", "COMPANY_NAME", "VOICE_LANGUAGE",
                "AUDIO_SAMPLE_RATE", "AUDIO_CHUNK_SIZE", "AUDIO_FORMAT",
                "ARI_BASE_URL", "ARI_USERNAME", "ARI_PASSWORD",
                "EXTERNAL_MEDIA_HOST", "EXTERNAL_MEDIA_PORT",
                "MAX_TOKENS", "TEMPERATURE", "VAD_ENERGY_THRESHOLD"
            ]
            
            # Create clean environment patch
            clean_env = {"OPENAI_API_KEY": "test-key"}
            for var in env_vars_to_clear:
                if var in os.environ:
                    clean_env[var] = ""
            
            with patch.dict(os.environ, clean_env, clear=False):
                # Create settings instance with clean environment
                settings = VoiceAssistantSettings()
                
                # AI Settings (test actual defaults from settings.py)
                assert settings.gemini_model == "gpt-4o-mini"
                assert settings.gemini_live_model == "gpt-4o-mini"
                ...
                assert settings.stasis_app == "openai-voice-assistant"
            ...
            if env_existed and temp_env_file.exists():
                shutil.move(temp_env_file, env_file) if temp_env_file.exists() else None
    ...
    def test_custom_settings_from_env(self):
        ...
        with patch.dict(os.environ, custom_env):
            ...
            assert settings.OPENAI_API_KEY == "custom-api-key"
...
class TestConfigurationValidation:
    ...
    def test_openai_voice_validation(self):
...
class TestConfigurationIntegration:
    ...
    def test_openai_config_compatibility(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            ...
            # Create OpenAIRealtimeConfig from settings
            openai_config = OpenAIRealtimeConfig(
                model=settings.gemini_live_model,
                voice=settings.gemini_voice
            )
            
            assert openai_config.model == settings.gemini_live_model
            assert openai_config.voice == settings.gemini_voice
