...
class IntegrationTestEnvironment:
    ...
    async def setup(self):
        ...
        self.openai_api = MockOpenAIRealtimeAPI()
        await self.openai_api.connect()
    ...
    async def teardown(self):
        ...
        # Stop services
        if self.openai_api:
            await self.openai_api.disconnect()
    ...
    async def simulate_conversation(
        ...
        for turn in conversation.turns:
            ...
            if turn.role.value == "user":
                ...
                if turn.audio_data:
                    await self.openai_api.send_audio_chunk(turn.audio_data)
                
                # Simulate speech detection
                await self.openai_api.simulate_speech_detection(True)
                await asyncio.sleep(0.1)
                await self.openai_api.simulate_speech_detection(False)
                
                # Commit audio buffer
                await self.openai_api.commit_audio_buffer()
            ...
            # Simulate assistant response
                ...
                # Generate response
                await self.openai_api.create_response()
                
                # Simulate audio response
                if turn.audio_data:
                    await self.openai_api.simulate_audio_response(turn.audio_data)
                
                await self.openai_api.simulate_response_complete()
