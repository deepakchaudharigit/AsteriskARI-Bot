...
try:
    # from src.voice_assistant.ai.openai_realtime_client import OpenAIRealtimeConfig
except ImportError:
...
def test_settings():
    ...
    class TestSettings:
        def __init__(self):
            ...
            self.stasis_app = "openai-voice-assistant"
            ...
            self.openai_model = "gpt-4o-realtime-preview-2024-10-01"
            self.openai_voice = "alloy"
            # Using OpenAI for AI integrationns = 150
...
@pytest.fixture
def openai_config():
    """Provide OpenAI Realtime configuration for tests."""
    # if OpenAIRealtimeConfig is None:
        # Fallback configuration for when module is not available
        class MockOpenAIRealtimeConfig:
            def __init__(self):
                self.model = "gpt-4o-realtime-preview-2024-10-01"
                self.voice = "Puck"
                self.input_audio_format = "pcm16"
                self.output_audio_format = "pcm16"
        return MockOpenAIRealtimeConfig()
...
def mock_openai_client():
    """Provide mock OpenAI client for tests."""
