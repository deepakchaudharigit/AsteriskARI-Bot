...
Tests the flow from Asterisk audio input through processing to OpenAI Realtime API.
"""
...
# Removed Gemini import - using OpenAI
...
class TestAudioPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_audio_processor_to_gemini_pipeline(self, audio_config, openai_config):
        """Test audio flow from processor to Gemini client."""
        # Setup components
        audio_processor = RealTimeAudioProcessor(audio_config)
        openai_client = OpenAIRealtimeClient(config=openai_config)
        
        # Mock Gemini connection
        openai_client.is_connected = True
        openai_client.websocket = AsyncMock()
        openai_client._send_event = AsyncMock()
        ...
        openai_client.send_audio_chunk = mock_send_audio
        
        # Setup audio processor callback to send to Gemini
        async def audio_chunk_callback(data):
            audio_data = data["audio_data"]
            await openai_client.send_audio_chunk(audio_data)
    ...
    @pytest.mark.asyncio
    async def test_speech_detection_to_response_pipeline(self, audio_config, openai_config):
        ...
        openai_client.websocket = AsyncMock()
        openai_client._send_event = AsyncMock()
        openai_client.commit_audio_buffer = AsyncMock(return_value=True)
        openai_client.create_response = AsyncMock(return_value=True)
    ...
    @pytest.mark.asyncio
    async def test_openai_response_to_audio_output(self, audio_config, openai_config):
        ...
        async def openai_response_handler(data):
            audio_data = data["audio_data"]
        ...
        openai_client.register_event_handler("audio_response", openai_response_handler)
        
        # Simulate Gemini response
        test_response_audio = AudioGenerator.generate_speech_like(500)  # 500ms response
        
        # Trigger response handler
        await openai_response_handler({
    ...
    @pytest.mark.asyncio
    async def test_bidirectional_audio_flow(self, audio_config, openai_config):
        ...
        openai_client.websocket = AsyncMock()
        openai_client._send_event = AsyncMock()
        openai_client.send_audio_chunk = AsyncMock(return_value=True)
        ...
        openai_client.send_audio_chunk.assert_called_once_with(input_audio)
        
        await audio_processor.stop_processing()
    
    @pytest.mark.asyncio
    async def test_audio_format_consistency(self, audio_config, openai_config):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_performance(self, audio_config, openai_config, performance_thresholds):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_error_recovery(self, audio_config, openai_config):
    ...
    @pytest.mark.asyncio
    async def test_pipeline_with_session_manager(self, audio_config, openai_config):
