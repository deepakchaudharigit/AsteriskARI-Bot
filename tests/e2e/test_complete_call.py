...
class TestCompleteCallWorkflow:
        ...
        try:
            ...
            stasis_start_event = {
                ...
                "channel": {
                    ...
                    "dialplan": {"context": "openai-voice-assistant", "exten": "1000"}
                }
            }
    ...
    async def test_call_with_audio_processing(self, test_settings):
        ...
        handler._start_external_media = AsyncMock(return_value=True)
        handler.openai_client.start_conversation = AsyncMock(return_value="session-123")
        handler.openai_client.send_audio_chunk = AsyncMock(return_value=True)
        handler.openai_client.commit_audio_buffer = AsyncMock(return_value=True)
        handler.openai_client.create_response = AsyncMock(return_value=True)
        ...
        try:
            ...
            handler.openai_client.send_audio_chunk.assert_called()
    ...
    async def test_call_with_interruption(self, test_settings):
        ...
        handler._start_external_media = AsyncMock(return_value=True)
        handler.openai_client.start_conversation = AsyncMock(return_value="session-123")
        handler.openai_client.cancel_response = AsyncMock(return_value=True)
...
class TestCallReliability:
    ...
    async def test_network_interruption_recovery(self, test_settings):
        ...
        async def mock_openai_connect():
            nonlocal connection_attempts
            connection_attempts += 1
            return connection_attempts > 1  # Fail first attempt, succeed second
        
        handler.openai_client.connect = mock_openai_connect
        handler.external_media_handler.start_server = AsyncMock(return_value=True)
        handler._answer_call = AsyncMock(return_value=True)
        handler._start_external_media = AsyncMock(return_value=True)
        handler.openai_client.start_conversation = AsyncMock(return_value="session-123")
