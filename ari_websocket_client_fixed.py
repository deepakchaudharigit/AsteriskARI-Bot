...
class ARIWebSocketClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Correct ARI WebSocket endpoint format
            ws_url = f"{self.ari_url}/asterisk/ari/events?{urlencode({'app': self.app})}"
            ...
            # Connect with basic auth in URL (alternative method)
            auth_url = f"ws://{self.username}:{self.password}@localhost:8088/asterisk/ari/events?{urlencode({'app': self.app})}"
...
async def main():
    ...
    if await client.connect():
        ...
        print("Check Stasis registration with:")
